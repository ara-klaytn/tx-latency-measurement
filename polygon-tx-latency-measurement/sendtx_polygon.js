...
async function sendTx() {
  ...
  try {
    ...
    const gasStationResult = await fetch(process.env.GAS_STATION_URL);
    const gasStationData = await gasStationResult.json();

    const maxPriorityFeePerGas = gasStationData.standard.maxPriorityFee;
    const maxFeePerGas = gasStationData.standard.maxFee;

    //const maxPriorityFeePerGas = web3.utils.toHex(Math.round(gasStationData.standard.maxPriorityFee * 1e9))
    //const maxFeePerGas = web3.utils.toHex(Math.round(gasStationData.standard.maxFee * 1e9))
